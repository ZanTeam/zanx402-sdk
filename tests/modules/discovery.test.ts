...
describe('DiscoveryModule', () => {
  ...
  it('health() returns health status', async () => {
    ...
    const discovery = new DiscoveryModule(mockHttp);
    const result = await discovery.health();

    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.HEALTH);
  ...
  it('listProviders() returns providers array', async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.PROVIDERS);
    expect(result).toEqual(expectedData);
  });
  ...
  it('listNetworks() returns networks array', async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.NETWORKS);
    expect(result).toEqual(expectedData);
  });
  ...
  it('listBundles() returns bundles array', async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.BUNDLES);
    expect(result).toEqual(expectedData);
  });
  ...
  it('getX402Capability() returns capability object', async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.WELL_KNOWN);
    expect(result).toEqual(expectedData);
  });
  ...
  it('getLlmsTxt() returns text string', async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith(ENDPOINTS.LLMS_TXT);
